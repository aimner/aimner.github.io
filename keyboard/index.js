...
const keyboardRawsRu = [
  ["Backspace", "=", "-", "0", "9", "8", "7", "6", "5", "4", "3", "2", "1", "ё"],
  ...
  changeButtonText() {
    for (let i = 0; i < keyboardRaws.length; i++) {
      for (let j = 0; j < keyboardRaws[i].length; j++) {
        lang
          ? (buttonsArr[i][j].innerHTML = keyboardRawsRu[i][j])
          : (buttonsArr[i][j].innerHTML = keyboardRaws[i][j]);
      }
    }
  }
...
function increaseTextShiftRu(event) {
  for (let i = 0; i < keyboardRaws.length; i++) {
    for (let j = 0; j < keyboardRaws[i].length; j++) {
      buttonsArr[i][j].innerHTML === lettersShiftRu[i][j]
        ? console.log("lol")
        : (buttonsArr[i][j].innerHTML = lettersShiftRu[i][j]);
      if (
        (event.type === "mousedown" || event.type === "keydown") &&
        buttonsArr[i][j].innerHTML.length <= 1
      ) {
        buttonsArr[i][j].innerHTML = buttonsArr[i][j].innerHTML.toUpperCase();
      }
      if (
        (event.type === "mouseup" || event.type === "keyup") &&
        buttonsArr[i][j].innerHTML.length <= 1
      ) {
...
function increaseTextShiftEn(event) {
  for (let i = 0; i < keyboardRaws.length; i++) {
    for (let j = 0; j < keyboardRaws[i].length; j++) {
      buttonsArr[i][j].innerHTML === lettersShift[i][j]
        ? console.log("lol")
        : (buttonsArr[i][j].innerHTML = lettersShift[i][j]);
      if (
        (event.type === "mousedown" || event.type === "keydown") &&
